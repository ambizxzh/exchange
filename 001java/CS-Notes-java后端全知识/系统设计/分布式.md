...
#### 1. Prepare 阶段(发送Prepare请求,会生成Prepare请求序号)
...
如果 Acceptor 接收到一个 Prepare 请求，包含的提议为 [n2, v2]，并且之前已经接收过提议 [n1, v1]。如果 n1 \> n2，那么就丢弃该提议请求；否则，**发送 Prepare 响应，该 Prepare 响应包含之前已经接收过的提议 [n1, v1]，设置当前接收到的提议为 [n2, v2]**，并且保证以后不会再接受序号小于 n2 的提议。**(接收到的Prepare请求的序号值大于Acceptor的本地值,那么把本地值的旧值作为响应的一部分发回给Proposer,且本地值更新为刚刚接收到的值  _zxzh; 否则(接收到的Prepare请求的序号值小于Acceptor的本地值),抛弃这个请求)**
