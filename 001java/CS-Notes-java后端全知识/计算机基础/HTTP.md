...
公开密钥所有人都可以获得，通信发送方(浏览器)获得接收方(服务器)的公开密钥之后，就可以使用公开密钥进行加密，接收方收到通信内容后使用私有密钥解密。

非对称密钥除了用来加密，还可以用来进行签名。因为私有密钥无法被其他人获取，因此通信发送方(服务器)使用其私有密钥进行签名，通信接收方(浏览器)使用发送方的公开密钥对签名进行解密，就能判断这个签名是否正确。
