...
[systemctl命令 – 管理系统服务 – Linux命令大全(手册) (linuxcool.com)](https://www.linuxcool.com/systemctl)
...
- **`pwd`：** 显示当前所在位置,print working directory

- `sudo + 其他命令`：以系统管理者的身份执行指令，也就是说，经由 sudo 所执行的指令就好像是 root 亲自执行。

- **`grep 要搜索的字符串 要搜索的文件 --color`：** 搜索命令，--color 代表高亮显示.（grep: global search regular expression(RE) and print out the line，全面搜索正则表达式并把行打印出来）
